			...
			unsigned long OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	LagBufferSpikeCount = (int)(InputCellCount * (mFrameLength_Seconds * 1000.0));
	LagSize_Seconds = mMaxLag_Seconds / (double)NumLags;
	InputIpsi = new SpikeStream *[NumLags];
	DelayBuffers[0] = NULL;

	for (int Lag = 1; Lag < NumLags; Lag++)
		DelayBuffers[Lag] = new SpikeDelayBuffer((int)(LagBufferSpikeCount + (InputCellCount * LagSize_Seconds * Lag * 2000)), LagSize_Seconds * Lag, mFrameLength_Seconds, mSampleRate_Hz);
	...
	for (Sample = 0; Sample < SamplesPerFrame; Sample++)
	{
		for (Channel = 0; Channel < NumChannels; Channel++)
		{
			...
			for (Lag = 0; Lag < NumLags; Lag++)
			{
				...
				ExcitatorySynapses[Channel][Lag]->Fire(InputIpsi[Lag]->CountSpikes(SynapseList[Channel], NumInputReps));
				// ExcitatorySynapses synapse for contralateral spikes received in the current sample period
				ExcitatorySynapses[Channel][Lag]->Fire(ContraSpikeCount);
