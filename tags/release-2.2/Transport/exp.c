...
INTERNAL MODULE_HOSTLIST *ModList[MAX_MODULES];
...
main(int argc, char *argv[]) {
	...
	for (i = 0; i < Nmodules; ++i) {
		...
		for (j = 0; j < nmod; ++j) {
			printf(" %s", modlocs[j].host);
		}
		printf("\n");


		/* Copy the host locations into our module array */
	...
	for (i = 0; i < Nmodules; ++i) {
		...
		k  = count++ % ModList[j]->nhost;
		mh = &ModList[j]->host[k];

		/* Build the command string */
		sprintf(args, "-inst %s -exp %s %s",
			Modules[i].instname,
			expname,
			expdaff);
