...
main(int argc, char *argv[]) {
	...
	Pgm = argv[0];  if (streq(Pgm, "app_main")) Pgm = "viewspike";
	++argv; --argc;
...
#endif
		if (!probe) {
			readfile(Dataf, xs*(NChan?NChan:1));
		}
...
printf("range %f - %f: xscale = %f\n", MinTime, MaxTime, xscale);
...
/****
if (i < 5) printf("Spike [%d (%d B)]: %d @ %f\n", i, sizeof(*d), d->cell, d->time);
****/
		Data[j].cell = d->cell;
		Data[j].time = d->time;
		++j;
