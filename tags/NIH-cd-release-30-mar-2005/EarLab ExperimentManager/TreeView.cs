...
namespace EarLab
{
	...
	public class TreeView : System.Windows.Forms.Form
	{
		...
		public TreeView()
		{
			...
			RegistryInit();
			RunFile = theRunfileName.Value;
		}

		private void RegistryInit()
		{
			theKey = Registry.LocalMachine.CreateSubKey(@"Software\EarLab");
			theRunfileName = new RegistryString(theKey, "RunfileName", Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), @"EarLab\runfile.xml"));
			...
			for (int CurRun = 0; CurRun < theRunFile.Run.Length; CurRun++)
			{
				...
				for (int InputFile = 0; InputFile < theRunFile.Run[CurRun].SourceFileNames.Length; InputFile++)
				{
					...
					if (File.Exists(((NodeInfo)(treeView1.Nodes[CurRun].Nodes[InputDirectory].Nodes[InputFile].Tag)).FileName))
						treeView1.Nodes[CurRun].Nodes[InputDirectory].Nodes[InputFile].ForeColor = Color.Green;
					else
						treeView1.Nodes[CurRun].Nodes[InputDirectory].Nodes[InputFile].ForeColor = Color.Red;
				}
				for (int OutputFile = 0; OutputFile < theRunFile.Run[CurRun].OutputFileNames.Length; OutputFile++)
				{
					...
					if (File.Exists(((NodeInfo)(treeView1.Nodes[CurRun].Nodes[OutputDirectory].Nodes[OutputFile].Tag)).FileName))
						treeView1.Nodes[CurRun].Nodes[OutputDirectory].Nodes[OutputFile].ForeColor = Color.Green;
					else
						treeView1.Nodes[CurRun].Nodes[OutputDirectory].Nodes[OutputFile].ForeColor = Color.Red;
				}
			}
			treeView1.EndUpdate();
		...
		private void treeView1_BeforeExpand(object sender, System.Windows.Forms.TreeViewCancelEventArgs e)
		{
			...
			for (int Subnodes = 0; Subnodes < e.Node.Nodes.Count; Subnodes++)
			{
				...
				{
					if (File.Exists(curInfo.FileName))
						e.Node.Nodes[Subnodes].ForeColor = Color.Green;
					else
						e.Node.Nodes[Subnodes].ForeColor = Color.Red;
				}
			}
		}
