			...
			unsigned long OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	if (OutputTypes[0] != WaveformData)
		throw EarlabException("%s: Currently this module can only handle waveform data streams.  Sorry!", ModuleName());
	...
	for (int x = 0; x < mDimension[0]; x++)
	{
		for (int y = 0; y < mDimension[1]; y++)
		{
			for (int z = 0; z < mDimension[2]; z++)
			{
				...
				{
				case 1:
					Output->Data(x) = NewSample;
					break;
				case 2:
					Output->Data(x, y) = NewSample;
					break;
				case 3:
					Output->Data(x, y, z) = NewSample;
					break;
				default:
					throw EarlabException("%s: Number of output dimensions must be 1, 2, or 3", ModuleName());
					break;
				}
			}
		}
