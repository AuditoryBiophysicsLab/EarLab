...
int Periodicity::Start(int NumInputs, int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], int NumOutputs, int OutputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions])
{
	int i;
...
int Periodicity::Advance(DoubleMatrixN *Input[EarlabMaxIOStreamCount], DoubleMatrixN *Output[EarlabMaxIOStreamCount])
{
	...
	if ((Output[0]->Rank(1) != mNumChannels) || (Output[0]->Rank(2) != mNumBMFChannels))
	...
	for (CurSample = 0; CurSample < mFrameSize_Samples; CurSample++)
	{
		for (CurFreqChannel = 0; CurFreqChannel < mNumChannels; CurFreqChannel++)
		{
			...
			CurDataPoint = Input[0]->Data(CurSample, CurFreqChannel);
			for (CurBMFChannel = 0; CurBMFChannel < mNumBMFChannels; CurBMFChannel++)
			{
				...
				Output[0]->Data(CurSample, CurFreqChannel, CurBMFChannel) = MembraneVoltage * CurDataPoint;
				ItemCount++;
			}
		}
		OutputLagNumber++;
