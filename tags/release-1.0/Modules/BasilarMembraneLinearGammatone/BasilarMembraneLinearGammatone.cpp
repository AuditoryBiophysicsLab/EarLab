...
int BasilarMembraneLinearGammatone::Start(int NumInputs, int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], int NumOutputs, int OutputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions])
{
    ...
    mDeltaT = (float)(1.0 / mSampleRate_Hz);
...
# define WR_TEXT "wt"
	...
	for (i = 0; i < mNumChannels; i++)
    {
		...
		mCFArray.Data(i) = (float)(CurrentCF_Hz * 2.0 * PI);
        ...
        mBMA.Data(i) = (float)((1.0 / ((mTauArray.Data(i) * 2.0)/mDeltaT + 1)));
        mBMB.Data(i) = (float)(mTauArray.Data(i) * 2.0 / mDeltaT - 1);
        mCFShift.Data(i) = (float)(mCFArray.Data(i) * mDeltaT);
    } 
	if (mOutputFile != NULL)
		fclose(mOutputFile);
...
int BasilarMembraneLinearGammatone::Advance(DoubleMatrixN *Input[EarlabMaxIOStreamCount], DoubleMatrixN *Output[EarlabMaxIOStreamCount])
{
...
/******************** FREQUENCY SHIFT DOWN **********************************/
			...
			//mShiftIn.Data(i) = Input[0].Data(j) * Complex::eToTheI(mCFShift.Data(i) * mCounter);
            //shftin[i].IMG  = input[j] * sin(-1 * cfshift[i] * counter);
			mShiftIn.Data(i).Imag = (float)(Input[0]->Data(j) * sin(-1 * mCFShift.Data(i) * mCounter));
            //shftin[i].REAL = input[j] * cos(cfshift[i] * counter);
			mShiftIn.Data(i).Real = (float)(Input[0]->Data(j) * cos(mCFShift.Data(i) * mCounter));
