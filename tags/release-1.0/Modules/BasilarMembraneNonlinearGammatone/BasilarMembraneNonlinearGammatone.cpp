...
int BasilarMembraneNonlinearGammatone::Start(int NumInputs, int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], int NumOutputs, int OutputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions])
{
...
int BasilarMembraneNonlinearGammatone::Advance(DoubleMatrixN *Input[EarlabMaxIOStreamCount], DoubleMatrixN *Output[EarlabMaxIOStreamCount])
{
    int i,j;
    ...
    {
		CurInputSample = Input[0]->Data(j) * mInputScaleFactor;
        for (i = 0; i < mNumChannels; i++)
        {
			Output[0]->Data(j, i) = mLowpassFilter[i].Filter(
				mDRNL[i].Filter(CurInputSample, mDisableNonlinearPath)) * mOutputScaleFactor;
		}
    }
	Data = Output[0]->GetData(&DataLen);
	mWriter->CheckMinMax(Data, DataLen);
	mSampleCount += mFrameSize_Samples;
    return (mFrameSize_Samples * mNumChannels) + 1;
}
