...
public:
	HHChannelPopulation();
	HHChannelPopulation(double InitialVoltage_mV, double Temperature_DegreesC);
	virtual void CalculateRates(double Voltage_mV) = 0;
	void CalculateSteadyState(void);

	double MaxConductance_Siemens;		// Set up by the derived class constructor
	double ReversalPotential_mV;		// Set up by the derived class constructor
	double alpha, beta;					// Activation and inactivation rates
	double activation;					// Current conductance activation value (0..1)
	double SteadyStateActivation;		// Set up by the derived class constructor

	virtual double GetDerivative(double IntermediateY, double InstantaneousVoltage_mV);
...
{
public:
	HH_Kslow(double InitialVoltage_mV, double Temperature_DegreesC);
	virtual void CalculateRates(double Voltage_mV);
};

class HH_Kfast : public HHChannelPopulation
{
public:
	HH_Kfast(double InitialVoltage_mV, double Temperature_DegreesC);
	virtual void CalculateRates(double Voltage_mV);
};
...
{
public:
	HH_NaActivation(double InitialVoltage_mV, double Temperature_DegreesC);
	virtual void CalculateRates(double Voltage_mV);
};
...
{
public:
	HH_NaInactivation(double InitialVoltage_mV, double Temperature_DegreesC);
	virtual void CalculateRates(double Voltage_mV);
};
