...
void BinaryWriter::Write(const double *buf, unsigned long item_count)
{
	unsigned long i;
	...
	fwrite(buf, sizeof(double), item_count, BinaryFP);
}

void BinaryWriter::CheckMinMax(const double *buf, unsigned long item_count)
{
	unsigned long i;
...
{
	if (!IsOpen)
		throw EarlabException("BinaryWriter: Attempted to write double precision metadata (%s) to unopened BinaryWriter object", tag);
	if (MetadataFP == NULL)
		OpenMetadataFile();
	fprintf(MetadataFP, "double\t%s\t%f\n", tag, value);
}
