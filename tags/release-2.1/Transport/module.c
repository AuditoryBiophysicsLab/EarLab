...
INTERNAL void do_param(EVENT_EFI_PARAMETER_FILE *eparam);
...
INTERNAL char       ModName[200];       /* Module print name/instance   */
...
main(int argc, char *argv[]) {
	...
	printf("%s: %s.%s joined %s as client id %d (%d clients)\n",
	       Pgm, MODULENAME, InstName, server, id, nc);
	gethostname(me, sizeof(me));
	printf("%s: %s.%s loaded on host %s\n", Pgm, MODULENAME, InstName, me);
	...
	/* Register ourselves */
	event_register("earlab", MODULENAME, InstName);

	/* Set Module's Instance Name */
	ModuleSetName(InstName);

	/* Construct module name to be used in print message */
	if (streq(MODULENAME, InstName)) strcpy(ModName, MODULENAME);
	else sprintf(ModName, "%s.%s", InstName, MODULENAME);
	...
	while (!done) {
		...
		switch(e.ev_head.type) {
		  ...
		  case ET_EFI_PARAMETER_FILE:
			do_param((EVENT_EFI_PARAMETER_FILE *)&e);
			break;
...
INTERNAL void
do_param(EVENT_EFI_PARAMETER_FILE *eparam) {
	char *file, *section;
	int   rc;
	...
	if (Debug) printf("%s.do_param: file %s\n", ModName, file);

	if (!*file) {
		rc = ModuleReadParameters(NULL, NULL);
		send_status(EFI_STATUS_PARAM, rc?EFI_STATUS_READY:EFI_STATUS_ERROR, 0);
		return;
	}

	if (Debug)
	   printf("%s.do_param: requesting download %s\n", ModName, file);
	...
	if (!rc) {
		...
		errno = efs_error();
		printf("%s.do_param: download failure %d for %s - %s\n",
			ModName, errno, file, efs_errormsg(errno));
	...
	for (i = 0; i < ninput; ++i) {
		...
		if      (type == EFI_STREAM_WAVEFORM) nmatrix += nelem;
			...
			printf("%s.config: unknown input arg %d type %d\n",
				ModName, i+1, econf->input_type[i]);
...
printf("%s.config: allocating %d bytes for 2 x %d inputs, %d outputs\n", ModName, nbyte, ninput, noutput);
    ...
    if (Debug)
     printf("%s.conf: Constructed bank 0, input %d, MatrixN w/ %d dim %dx%dx%d\n",
	    ModName, i, *(InArg0[i].data.waveform->mDimCount), InRank[i][0], InRank[i][1], InRank[i][2]);
    ...
    if (Debug)
     printf("%s.conf: Constructed bank 1, input %d, MatrixN w/ %d dim %dx%dx%d\n",
	    ModName, i, *(InArg1[i].data.waveform->mDimCount), InRank[i][0], InRank[i][1], InRank[i][2]);
    ...
    if (Debug)
     printf("%s.conf: Constructed bank 1, input %d, Spike max %d\n",
	    ModName, i, InArg1[i].data.spike->MaxSpikes);
			break;
    ...
    if (Debug)
     printf("%s.conf: Constructed output %d, MatrixN w/ %d dim %dx%dx%d\n",
	    ModName, i, *(OutArg[i].data.waveform->mDimCount), OutRank[i][0], OutRank[i][1], OutRank[i][2]);
    ...
    if (Debug)
     printf("%s.conf: Constructed ouput %d, Spike max %d\n",
	    ModName, i, OutArg[i].data.spike->MaxSpikes);
			break;

		  default: /* Should never happen since we already checked it */
			printf("%s: Fatal error - output %d of unknown type %d\n",
				ModName, i, econf->output_type[i]);
			abort();
		}
	...
	if (Dump) for (i = 0; i < Noutput; ++i) {
		...
		else          strcat(basename, MODULENAME);
		sprintf(oname, ".%d.out", i+1);

		strcpy(filename, basename);
		strcat(filename, oname);

		Dumpf[i] = fopen(filename, "w");
		...
		if (!Dumpf[i] || !Metaf[i]) {
			fprintf(stderr,
				"%s: can't open file \"%s\" for metafile output %d\n",
				ModName, filename, i+1);
...
printf("%s.pre-start: input.%d %s %d elems\n", ModName, i+1, StreamTypeName(InType[i]), InSize[i]);
	}
...
printf("%s.pre-start: output.%d %s %d elems\n", ModName, i+1, StreamTypeName(OutType[i]), OutSize[i]);
	}
...
send_status(int type, int status, int frame) {
	...
	if (Verbose || Debug)
	   printf("%s: Sending status %s %s (%d) for frame %d\n",
		  ModName, tmsg, smsg, estatus.status, estatus.frame);
...
do_wavedata(EVENT_EFI_WAVEDATA *edata) {
   ...
   if (Debug)
      printf("%s.rcvdata: Rcv input stream %d from %d for input %d, for frame %d, using buffer %d at 0x%08x (%d bytes)\n",
	      ModName, edata->tag, from, inarg, edata->frame,
   ...
   if (Debug) {
	...
	data = edata->data;
	 printf("%s.rcvdata: input data %f %f %f %f ...\n",
		ModName, data[0], data[1], data[2], data[3]);
   }
   ...
   if (Debug)
	printf("%s.rcvdata: Added elems %d-%d of %d to buf %d\n",
		ModName, edata->index0, edata->indexN, edata->elems, indx);
...
if (Debug)
   printf("%s.advance: running module at timestep %d using buffer set %d\n", ModName, Frame, Use);
	...
	if (Debug) for (i = 0; i < Ninput; ++i) {
		...
		printf("%s.advance: input.%d ", ModName, i);
	...
	if (Debug) for (i = 0; i < Noutput; ++i) {
		...
		printf("%s.advance: output.%d ", ModName, i);
		show_desc(stdout, stream);
		printf(": ");
		show_data(stdout, stream, 4);
		printf(" ...\n");
...
if (Debug) {
   printf("%s.advance: delivering output.%d, tag %d, to %d (%d elems)\n",
	ModName, i, Obuf[i]->tag, OutDest[i], Obuf[i]->elems);
	printf("%s.advance: output.%d data ", ModName, i+1);
...
if (Debug)
   printf("%s.senddata: sending to %d, frame %d, tag %d, %d-%d of %d elems (%d bytes)\n",
	  ModName, frame, tag, to, edata->index0, edata->indexN, nelem, size);
...
dump_data(FILE *f, EVENT_EFI_DATA *obuf) {
	...
	fwrite(obuf->data, elemsize, obuf->elems, f);
}
	...
	switch (ectl->type) {
	  case EFI_CTL_QUERY:
		printf("%s.ctl: from %d, query %d\n",
		       ModName, EventFrom(ectl), ectl->param);
	  ...
	  case EFI_CTL_STATUS:
		printf("%s.ctl: from %d, status\n",
			ModName, EventFrom(ectl));

		break;

	  default:
		printf("%s.ctl: from %d, unknown type %d, flag %d\n",
			ModName, EventFrom(ectl), ectl->type, ectl->tag);
...
printf("%s.do_probe: %s output.%d->%d (tag %d)\n",
	ModName, enable?"add":"delete", param, to, tag);
