...
int MSOXcorr::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	if ((Output->Rank(0) != mNumChannels) || (Output->Rank(1) != mMaxLag_Samples))
	...
	for (i = 0; i < mNumChannels; i++)
	{
		...
		for (k = 0; k < mFrameSize_Samples; k++)
		{
			...
			else
			{
				...
				mIpsilateral[k] = InputIpsi->Data(CurIndex, i);
				mContralateral[k] = InputContra->Data(CurIndex, i);
			}
#else
	...
	for (i = 0; i < mFrameSize_Samples; i++)
	{
		...
		{
			mFrameBuf[0].Data(i, k) = InputIpsi->Data(i, k);
			mFrameBuf[1].Data(i, k) = InputContra->Data(i, k);
		}
	}
#endif
