...
void TestBushyCell(int argc, char* argv[])
{
	...
	//TestSpikeStream();
	//return 0;
	//TestBushyCell_CurrentClamp(BUSHY_CURRENT);
	//TestBushyCell_Spike((18.2 * 18.2) / 19.5);
	TestBushyCell_Spike(d);
}
...
void TestBushyCell_Spike(double EPSPStrength)
{
	...
	HHCompartment SpikeCompartment(100000.0, -60.0e-3, 38.0);	// 38 degrees C for spike tests
	...
	for (i = 0; i < BUSHY_SPIKE_STEPS; i++)
	{
		...
		SpikeCompartment.GetActivations(&(K_slow[i]), &(K_fast[i]), &(Na_act[i]), &(Na_inact[i]));
	}
...
void TestBushyCell_CurrentClamp(double CurrentClamp_Amperes)
{
	...
	HHCompartment ClampCompartment(100000.0, -60.0e-3, 22.0);	// 22 degrees C for current clamp test
	...
	for (i = 0; i < BUSHY_CLAMP_STEPS; i++)
	{
		...
		ClampCompartment.GetActivations(&(K_slow[i]), &(K_fast[i]), &(Na_act[i]), &(Na_inact[i]));
	}
