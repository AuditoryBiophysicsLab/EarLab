...
StapediusReflexDriver::StapediusReflexDriver()
{
    memset(this, 0, sizeof(StapediusReflexDriver));
	SetModuleName("StapediusReflexDriver");
}
...
int StapediusReflexDriver::ReadParameters(const char *ParameterFileName)
{
	if (ModuleName() == NULL)
	    return ReadParameters(ParameterFileName, "StapediusReflexDriver");
	else
	    return ReadParameters(ParameterFileName, ModuleName());
}

int StapediusReflexDriver::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
    ...
    Status = theParamFile.GetParameter(SectionName, "LowpassCornerFrequency_Hz", mLowpassCornerFrequency_Hz, 20.0);
	Status = theParamFile.GetParameter(SectionName, "LowpassFilterOrder", mLowpassFilterOrder, 4);
	Status = theParamFile.GetParameter(SectionName, "Gain", mGain, 1);
    return 1;
}

int StapediusReflexDriver::Start(int NumInputs,  EarlabDataStreamType InputTypes[EarlabMaxIOStreamCount],  int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], 
...
int StapediusReflexDriver::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
    int counter = 0;
	double total;
	int i, samp, chan;
	...
	for (samp = 0; samp < mFrameSize_Samples; samp++)
	{
		...
		total /= mNumChannels;	// Normalize by the number of frequency channels
...
int StapediusReflexDriver::Stop(void)
{
...
int StapediusReflexDriver::Unload(void)
{
