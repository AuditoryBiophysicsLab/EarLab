...
int AudioMixer::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
	...
	mNumInputs = EarlabMaxIOStreamCount;
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	for (i = 0; i < NumInputs; i++)
	{
		if ((InputSize[i][0] != 0) && (InputSize[i][2] != 0))
			throw EarlabException("%s: All input streams to this module must be one- or two-dimensional.  Input %d has more than two.", ModuleName(), i);
		
		DimSize[i] = InputSize[i][1];

		if (InputSize[i][0] != OutputSize[0][0])
			throw EarlabException("%s: All input streams to this module must have the same length as the output stream.  Input %d's length is different.", ModuleName(), i + 1);
	}
...
int AudioMixer::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	for (i = 0; i < mNumSamples; i++)
	{
		...
		{
			if (DimSize[j] == 0)
				TempSample = Input[j]->Data(i);
			else
			{
				TempSample = 0.0;
				for (k = 0; k < DimSize[j]; k++)
				{
					TempSample += Input[j]->Data(i, k);
				}
			}
			CurSample += TempSample * InputScaleFactorArray[j];
		}
