...
BushyCell2::BushyCell2()
{
    memset(this, 0, sizeof(BushyCell2));
	SetModuleName("BushyCell2");
}
...
int BushyCell2::ReadParameters(const char *ParameterFileName)
{
	if (ModuleName() == NULL)
	    return ReadParameters(ParameterFileName, "BushyCell2");
	else
	    return ReadParameters(ParameterFileName, ModuleName());
}

int BushyCell2::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
...
int BushyCell2::Start(int NumInputs,  EarlabDataStreamType InputTypes[EarlabMaxIOStreamCount],  int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions],
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	for (Channel = 0; Channel < NumChannels; Channel++)
	{
		...
		for (int OutputRep = 0; OutputRep < NumOutputReps; OutputRep++)
		{
			...
			//BushyCells[Channel][OutputRep] = new BUSHY_CELL_TYPE(mSampleRate_Hz, -60.0e-3, mCellTemperature_DegreesC, newmResponseType, mTauAHP_Seconds, mAHPIncrement_Siemens);
...
int BushyCell2::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	int counter = 0;
	SpikeStream *Input;
	SpikeStream *Output;
...
//				BushyCells[Channel][Rep]->GetConductances_nS(ge_nS, gi_nS, gNa_nS, gKA_nS, gKHT_nS, gKLT_nS, gh_nS, gAHP_nS);
