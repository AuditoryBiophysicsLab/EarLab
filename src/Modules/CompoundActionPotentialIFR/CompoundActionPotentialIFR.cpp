...
int CompoundActionPotentialIFR::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
    ...
    //Status = theParamFile.GetParameter(SectionName, "LowpassCornerFrequency_Hz", mLowpassCornerFrequency_Hz, 5000.0);
	//Status = theParamFile.GetParameter(SectionName, "LowpassFilterOrder", mLowpassFilterOrder, 4);
    Status = theParamFile.GetParameter(SectionName, "ImpulseResponseFile", mImpulseResponseFilename, sizeof(mImpulseResponseFilename), NULL);
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	mFIRFilter = new FiniteImpulseResponse(mImpulseResponseFilename);
...
int CompoundActionPotentialIFR::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	for (samp = 0; samp < mFrameSize_Samples; samp++)
	{
		...
		total = mFIRFilter->Filter((float)total);

		//derivative = ((total - mOldSample) / mDeltaT) * mGain;
		//mOldSample = total;
