...
int DataSource::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
	...
	Status = theParamFile.GetParameter(SectionName, "CreateMetadataFile", mCreateMetadataFile, true);
	Status = theParamFile.GetParameter(SectionName, "WaveFileChannelNumber", mWaveFileChannelNumber, 0);
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	if (NumInputs != 0)
		printf("%s: Currently this module cannot handle input streams.  Sorry!\n", ModuleName());

	if (NumOutputs != 1)
		printf("%s: Currently this module can only handle one output stream.  Sorry!\n", ModuleName());
	...
	{
		OutputElementCounts[0] *= (unsigned int)(mFrameLength_Seconds * 2000);
	}
	...
	{
		if (mInputIsSpikeData)
			throw EarlabException("%s: Wave file input is not compatible with spike data output\n", ModuleName());

		if (mInputFileName != NULL)
			mInputFile = OpenInputFile(mInputFileName, "rb");

		if (mInputFile == NULL)
			throw EarlabException("%s: Error opening specified input wave file: \"%s\"\n", ModuleName(), mInputFileName);
	...
	else
	{
		...
		if (mInputFile == NULL)
			throw EarlabException("%s: Error opening specified input file: \"%s\"\n", ModuleName(), mInputFileName);
	}

	if (mCreateMetadataFile)
	{
...
int DataSource::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	else
	{
		...
		for (int x = 0; x < mDimension[0]; x++)
		{
			for (int y = 0; y < mDimension[1]; y++)
			{
				for (int z = 0; z < mDimension[2]; z++)
				{
					...
					else
					{
						...
						if (mCreateMetadataFile) mMetadataFile->UpdateMinMax(NewSample);
					}
					switch (mNumOutputDims)
					{
					case 1:
			...
			if (mCreateMetadataFile) mMetadataFile->AddSample();
		}
...
int DataSource::Stop(void)
{
	...
	if (mCreateMetadataFile) mMetadataFile->Write();
    if (mInputFile != NULL)
        CloseFile(mInputFile);
	mInputFile = NULL;
	return 1;
...
double DataSource::ReadWaveData(void)
{
	...
	case 24:
		...
		IntVal  = (mS24[(mWaveFileChannelNumber * 3) + 0] <<  0);	// LSB comes first
		IntVal |= (mS24[(mWaveFileChannelNumber * 3) + 1] <<  8);
		IntVal |= (mS24[(mWaveFileChannelNumber * 3) + 2] << 16);	// MSB comes last
		if (IntVal & 0x00800000)	// If the 24-bit sample is negative (bit 23 is set)
		{
			IntVal |= 0xFF000000;	// Sign-extend the 24 bit int to 32 bits 
		}
		value = (double)IntVal;
