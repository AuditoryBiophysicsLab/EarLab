...
CarneyInnerHaircell::CarneyInnerHaircell()
{
    memset(this, 0, sizeof(CarneyInnerHaircell));
	SetModuleName("CarneyInnerHaircell");
}
...
int CarneyInnerHaircell::ReadParameters(const char *ParameterFileName)
{
	if (ModuleName() == NULL)
	   return ReadParameters(ParameterFileName, "CarneyInnerHaircell");
	else
	    return ReadParameters(ParameterFileName, ModuleName());
}

int CarneyInnerHaircell::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
...
int CarneyInnerHaircell::Start(int NumInputs,  EarlabDataStreamType InputTypes[EarlabMaxIOStreamCount],  int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], 
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	mMetadataFile->SetSource(new XmlSource(ModuleName(), "CarneyInnerHaircell_0.1.0.0"));
	mMetadataFile->Write();
    return 1;
}

int CarneyInnerHaircell::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	int i, j;
...
int CarneyInnerHaircell::Stop(void)
{
...
int CarneyInnerHaircell::Unload(void)
{
