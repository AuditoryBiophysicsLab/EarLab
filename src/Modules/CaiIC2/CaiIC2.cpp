...
CaiIC2::CaiIC2()
{
    memset(this, 0, sizeof(CaiIC2));
	SetModuleName("CaiIC2");
}
...
int CaiIC2::ReadParameters(const char *ParameterFileName)
{
	if (ModuleName() == NULL)
	    return ReadParameters(ParameterFileName, "CaiIC2");
	else
	    return ReadParameters(ParameterFileName, ModuleName());
}

int CaiIC2::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
	...
	//Status = theParamFile.GetParameter(SectionName, "CompartmentClassName", mCompartmentClassName, sizeof(mCompartmentClassName), "Rothman03AHPCompartment");
...
int CaiIC2::Start(int NumInputs,  EarlabDataStreamType InputTypes[EarlabMaxIOStreamCount],  int InputSize[EarlabMaxIOStreamCount][EarlabMaxIOStreamDimensions], 
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	// Maybe because with the delayed SpikeStream must be held across frames.
	...
	for (Channel = 0; Channel < NumChannels; Channel++)
	{
		...
		for (int Lag = 0; Lag < NumLags; Lag++)
		{
			...
			if (newmResponseType==0)
				ICCells[Channel][Lag] = HHCompartmentFactory::Create(mCompartmentClassName, mSampleRate_Hz, -64.0e-3, mCellTemperature_DegreesC, newmResponseType, TYPE_II, mTauAHP_Seconds, mAHPIncrement_Siemens, mRothman03CustomParameterValues);
			else
				ICCells[Channel][Lag] = HHCompartmentFactory::Create(mCompartmentClassName, mSampleRate_Hz, -64.0e-3, mCellTemperature_DegreesC, newmResponseType, TYPE_II, mTauAHP_Seconds, mAHPIncrement_Siemens);
			// Six-argument constructor works only with Rothman03AHPCompartment
			//ICCells[Channel][Lag] = new COMPARTMENT_TYPE(mSampleRate_Hz, -64.0e-3, mCellTemperature_DegreesC, newmResponseType, mTauAHP_Seconds, mAHPIncrement_Siemens);
...
int CaiIC2::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	int counter = 0;
	...
	for (Sample = 0; Sample < SamplesPerFrame; Sample++)
	{
		for (Channel = 0; Channel < NumChannels; Channel++)
		{
			...
			for (Lag = 0; Lag < NumLags; Lag++)
			{
				...
				//ICCells[Channel][Lag]->GetConductances_nS(ge_nS, gi_nS, gNa_nS, gKA_nS, gKHT_nS, gKLT_nS, gh_nS, gAHP_nS);
