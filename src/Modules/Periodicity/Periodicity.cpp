...
Periodicity::~Periodicity()
{
	...
	for (i = 0; i < mNumChannels; i++)
	{
		...
		delete [] mExcitatoryFilters[i];
		delete [] mInhibitoryFilters[i];
	}
	if (mBMFArray_Hz != NULL)
		delete [] mBMFArray_Hz;
	delete mFrameBuffer;
	delete [] mExcitatoryFilters;
	delete [] mInhibitoryFilters;
}
...
int Periodicity::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
    ...
    Status = theParamFile.GetParameter(SectionName, "SpontaneousRateOffset", mSpontaneousRateOffset, 0);
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
    ...
    mNumBMFChannels = OutputSize[0][0];
	mDeltaT = 1.0 / mSampleRate_Hz;
	mBMFStep_Hz = log(mHighBMF_Hz / mLowBMF_Hz);
	mBMFArray_Hz = new double[mNumBMFChannels];
	mExcitatoryFilters = new NthOrderLowpass *[mNumChannels];
	mInhibitoryFilters = new NthOrderLowpass *[mNumChannels];
	...
	{
		mExcitatoryFilters[j] = new NthOrderLowpass[mNumBMFChannels];
		mInhibitoryFilters[j] = new NthOrderLowpass[mNumBMFChannels];
		for (i = 0; i < mNumBMFChannels; i++)
		{
			...
			mExcitatoryFilters[j][i].SetFilterOrder(mExcitatoryFilterOrder);
			mExcitatoryFilters[j][i].SetSampleRate_Hz(mSampleRate_Hz);
			mExcitatoryFilters[j][i].SetCornerFrequency_Hz(mBMFArray_Hz[i]);

			mInhibitoryFilters[j][i].SetFilterOrder(mInhibitoryFilterOrder);
			mInhibitoryFilters[j][i].SetSampleRate_Hz(mSampleRate_Hz);
			mInhibitoryFilters[j][i].SetCornerFrequency_Hz(mBMFArray_Hz[i] * (1.0 / mInhibitoryFrequencyRatio));
		}
	}
...
int Periodicity::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	if ((Output->Rank(0) != mNumBMFChannels) || (Output->Rank(1) != mNumChannels))
	...
	for (CurSample = 0; CurSample < mFrameSize_Samples; CurSample++)
	{
		for (CurFreqChannel = 0; CurFreqChannel < mNumChannels; CurFreqChannel++)
		{
			...
			for (CurBMFChannel = 0; CurBMFChannel < mNumBMFChannels; CurBMFChannel++)
			{
				ExcitatoryPotential = mExcitatoryFilters[CurFreqChannel][CurBMFChannel].Filter(CurDataPoint);
				InhibitoryPotential = mInhibitoryFilters[CurFreqChannel][CurBMFChannel].Filter(CurDataPoint);
				...
				mFrameBuffer[CurFreqChannel][CurBMFChannel] += (float)(MembraneVoltage * (CurDataPoint - mSpontaneousRateOffset));
			}
		}
	}
	for (CurBMFChannel = 0; CurBMFChannel < mNumBMFChannels; CurBMFChannel++)
	{
		for (CurFreqChannel = 0; CurFreqChannel < mNumChannels; CurFreqChannel++)
		{
			...
			Output->Data(CurBMFChannel, CurFreqChannel) = (float)Average;
			ItemCount++;
		}
	}
