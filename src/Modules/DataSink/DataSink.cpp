			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	{
		printf("%s: Currently this module can only handle either one waveform or one spike input data stream.  Sorry!\n", ModuleName());
	...
	if (mProduceOutputFile)
	{
		...
		if (mUseDownsampler)
		{
			if (!mBinaryOutput)
			{
				printf("%s: Error: Using the downsampler option requires binary output.  Set BinaryOutput to 'true'\n", ModuleName());
			...
			{
				printf("%s: Error: Downsampling requires a CF array to work properly\n", ModuleName());
