...
int BasilarMembraneLinearGammatone::ReadParameters(const char *ParameterFileName)
{
	return ReadParameters(ParameterFileName, ModuleName());
}

int BasilarMembraneLinearGammatone::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
    ...
    Status = theParamFile.GetParameter(SectionName, "UseGreenwoodFunction", mUseGreenwoodFunction, true);
...
/*****************************************************************************/
	...
	#define RD_TEXT	"rt"
#endif
	if (mUseGreenwoodFunction)
	{
#ifdef _WIN32
		_fmode = _O_TEXT;
#endif
		mCFFile = OpenOutputFile(mCFFileName, WR_TEXT);

		if (mCFFile == NULL)
			throw EarlabException("%s: Error opening OUTPUT CFArray file \"%s\"", ModuleName(), mCFFileName);
...
//		PlacemapStepSize_mm = mCochleaModeledLength_mm / (double)mNumChannels;
		if (mNumChannels == 1)
			PlacemapStepSize_mm = 0.0;
		else
			PlacemapStepSize_mm = mCochleaModeledLength_mm / ((double)mNumChannels - 1.0);
	...
	else
	{
		...
		_fmode = _O_TEXT;
#endif
		mCFFile = OpenInputFile(mCFFileName, RD_TEXT);

		if (mCFFile == NULL)
			throw EarlabException("%s: Error opening INPUT CFArray file \"%s\"", ModuleName(), mCFFileName);
		for (i = 0; i < mNumChannels; i++)
		{
			fscanf(mCFFile, "%f", mCFArray_Hz[i]);
			if (feof(mCFFile))
				throw EarlabException("%s: Error reading INPUT CFArray file \"%s\": Unexpected EOF while reading entry %d", ModuleName(), mCFFileName, i + 1);
		} // for (i = 0; i < mNumChannels; i++)
	} // if (mUseGreenwoodFunction) else
	if (mCFFile != NULL)
		CloseFile(mCFFile);

	for (i = 0; i < mNumChannels; i++)
	{
		mTauArray.Data(i) = mQArray.Data(i) / (mCFArray_Hz[i] * 2 * PI);
		mBMA.Data(i) = (double)((1.0 / ((mTauArray.Data(i) * 2.0)/mDeltaT + 1)));
		mBMB.Data(i) = (double)(mTauArray.Data(i) * 2.0 / mDeltaT - 1);
		mCFShift.Data(i) = (double)((mCFArray_Hz[i] * 2 * PI) * mDeltaT);
	} // for (i = 0; i < mNumChannels; i++)
	...
	mMetadataFile->SetSource(new XmlSource(ModuleName(), "BasilarMembraneLinearGammatone_0.1.0.0"));
	mMetadataFile->Write();
	return 1;
}
...
/****************** PUT BM OUTPUT INTO IHC ARRAY FOR NEXT STAGE ***********/
			...
			mMetadataFile->UpdateMinMax(data);
