...
int MSOXcorr::ReadParameters(const char *ParameterFileName, const char *SectionName)
{
    ...
    //Status = theParamFile.GetParameter(SectionName, "SamplesPerITDStep", mSamplesPerITDStep, 1);
    Status = theParamFile.GetParameter(SectionName, "ITDStepSize_uS", mITDStepSize_uS, (1e6/mSampleRate_Hz));
			...
			unsigned int OutputElementCounts[EarlabMaxIOStreamCount])
{
	...
	mNumITDSteps = (int)((mMaxITD_uS / mITDStepSize_uS) + 0.5); // The number of ITD values is mNumITDSteps + 1
	uSPerSample = 1e6 / mSampleRate_Hz;
	mSamplesPerITDStep = (int)((mITDStepSize_uS / uSPerSample) + 0.5); // Round off the sample count to the nearest integer
	// reset mITDStepSize_uS to be a multiple of the sample period
	mITDStepSize_uS = mSamplesPerITDStep * uSPerSample;
	// reset mITDStepSize_uS to be a multiple of the sample period
	mMaxITD_uS = mNumITDSteps * mITDStepSize_uS;

	//mMaxLag_Samples = (int)((mMaxITD_uS / (1e6 / mSampleRate_Hz)) + 0.5) + 1;	// Round off the sample count to the nearest integer
	mMaxLag_Samples = mNumITDSteps * mSamplesPerITDStep;
	ChkMaxLagSamples = mSamplesPerITDStep * (OutputSize[0][0] - 1);
    ...
    //if ((mSamplesPerITDStep * OutputSize[0][0]) != mMaxLag_Samples)
    ...
    if (ChkMaxLagSamples != mMaxLag_Samples)
        throw EarlabException("%s: Output lag dimension (%d) must match head size/sample rate (%d).  Sorry!", ModuleName(), 
			OutputSize[0][0], mMaxLag_Samples);
    ...
    mLag = new double[mNumITDSteps + 1];
	...
	mMetadataFile->Add(new XmlDimension("Interaural Time Difference (uS)", (mNumITDSteps + 1), 0, mMaxITD_uS));	// Linear lags from zero to mMaxITD_uS
...
int MSOXcorr::Advance(EarlabDataStream *InputStream[EarlabMaxIOStreamCount], EarlabDataStream *OutputStream[EarlabMaxIOStreamCount])
{
	...
	if ((Output->Rank(0) != (mNumITDSteps + 1)) || (Output->Rank(1) != mNumChannels))
		throw EarlabException("%s: Output size mismatch with Start()", ModuleName());
	...
	for (i = 0; i < mNumChannels; i++)
	{
		...
		NumLags = HalfCorrelate(mContralateral, mIpsilateral, mFrameSize_Samples, mLag, (mNumITDSteps + 1));
		for (k = 0; k < NumLags; k++)
		{
...
int MSOXcorr::HalfCorrelate(double ipsi[], double contra[], int n, double lag[], int shiftmax)
{
    int lagpos = 0;
    int cur_shift, cur_shift_Samples;
	double tmp;
	...
	{
		cur_shift_Samples = cur_shift * mSamplesPerITDStep;
		//tmp = ComputeLag(&ipsi[cur_shift], contra, n - cur_shift);
		tmp = ComputeLag(&ipsi[cur_shift_Samples], contra, n - cur_shift_Samples);
        lag[lagpos++] = tmp;
	}
...
double MSOXcorr::ComputeLag(double ipsi[], double contra[], int n)
{
    int i;
    double total = 0.0;
