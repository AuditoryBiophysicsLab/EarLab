...
INTERNAL char            *LocalExec[MAX_MODULES]; /* Local execution path   */

INTERNAL int              ExecMode   = EXEC_AGENT;
...
INTERNAL int  checkmod(char *dir, char *modname, char *execfile);
...
INTERNAL int   Execute(char *cmd, char *instname, char *cmdopts, char *server);
...
INTERNAL void  Fail(char *pgm, char *msg);
...
main(int argc, char *argv[]) {
	...
	while (argc && *argv[0] == '-') {
		...
		else if (substr(opt, "agentd"))    startagent = 1;
		else if (substr(opt, "noagentd"))  startagent = 0;
		...
		else if (substr(opt, "wd") || substr(opt, "rundir")) {
			modwd = argv[0];
			++argv; --argc;
		}
		...
		else if (substr(opt, "exec")) {
			char *mode;

			mode = argv[0];
			++argv; --argc;
			if      (substr(mode, "agentd")) ExecMode = EXEC_AGENT;
			else if (substr(mode, "local"))  ExecMode = EXEC_LOCAL;
			else if (substr(mode, "ssh"))    ExecMode = EXEC_SSH;
		}
		else {
			fprintf(stderr,
	...
	if (Nmodules == 0) {
		sprintf(errmsg,
			"no modules read from configuration file \"%s\"",
			 config);
		Fail(pgm, errmsg);
	}
	...
	if (streq(univs, "-search"))
	  if(!(univs = SearchWorld(METASERVER, EARLAB))) {
		sprintf(errmsg,
			"couldn't locate master server \"%s\" on metaserver \"%s\"",
			EARLAB, METASERVER);
		Fail(pgm, errmsg);
	  }
	...
	/*
	 * Construct module command line option arglist
	 */
	modopts[0] = '\0';
	strcat(modopts, tunnel?"-tunnel":"-notunnel");
	if (ModVerbose)                strcat(modopts, " -verbose");
	else                           strcat(modopts, " -quiet");
	if (dump)                      strcat(modopts, " -dump");
	for (i = 0; i < ModDebug; ++i) strcat(modopts, " -debug");
	for (i = 0; i < ModTrans; ++i) strcat(modopts, " -Debug");
	for (i = 0; i < ModFS; ++i)    strcat(modopts, " -fsdebug");
	if (expname) {
		char optstr[300];

		sprintf(optstr, " -experiment %s", expname);
		strcat(modopts, optstr);
	}
	if (modwd) {
		char optstr[300];

		sprintf(optstr, " -cd %s", modwd);
		strcat(modopts, optstr);
	}

	/*
	 * Non-agentd exec modes
	 */
	if (ExecMode == EXEC_LOCAL) {
		/* Make sure we can find all the modules */
		for (i = 0; i < Nmodules; ++i) {
			rc = checkmod(moddir, Modules[i].modname, execfile);
			if (!rc) {
				sprintf(errmsg,
					"Can't locate module executable \"%s:%s\"",
					Modules[i].instname,
					Modules[i].modname);
				Fail(pgm, errmsg);
			}
		...
		for (i = 0; i < Nmodules; ++i) {
			rc = checkmod(moddir, Modules[i].modname, execfile);
			if (!rc) {
				/* Whooo - it was just there!! */
				sprintf(errmsg, "Can't relocate module \"%s:%s\" at %s",
					Modules[i].instname,
					Modules[i].modname,
					execfile);
				Fail(pgm, errmsg);
			...
			Execute(execfile, Modules[i].instname, modopts, expdaff);
		}
		exit(0);
	}
	else if (ExecMode == EXEC_SSH) {
		/***
		  rc = exec_ssh();
		 ***/
		return(rc);
	...
	if (!id) {
		sprintf(errmsg, "couldn't join server \"%s\"\n", univs);
		Fail(pgm, errmsg);
	}
	...
	/* Initialize localexecution path */
	for (i = 0; i < Nmodules; ++i) LocalExec[i] = NULL;
	...
	for (i = 0; i < Nmodules; ++i) {
		...
		/* Check for running this module locally */
		if (streq(Modules[i].host, "-")) {
			/* Run locally */
			rc = checkmod(moddir, Modules[i].modname, execfile);
			if (!rc) {
				sprintf(errmsg,
					"Module \"%s\" not available in %s",
					Modules[i].modname, moddir);
				Fail(pgm, errmsg);
			}
			LocalExec[i] = (char *)malloc(strlen(execfile)+1);
			if (!rc) {
				sprintf(errmsg,
					"Couldn't allocate memory for buffer (%s)",
					execfile);
				Fail(pgm, errmsg);
			}
			strcpy(LocalExec[i], execfile);
		...
		if (nmod == 0) {
			sprintf(errmsg,
				"Module \"%s\" not available (as %s)",
				Modules[i].modname, mstr);
			Fail(pgm, errmsg);
		}
		...
		if (!mhl) {
			sprintf(errmsg,
				"ran out of memory after %d modules (needed %d bytes)!!",
				NModList, bytes);
			Fail(pgm, errmsg);
		}
		mhl->nexec = 0;
	...
	for (i = 0; i < Nmodules; ++i) {
		...
		/* See if we are starting this up locally */
		if (LocalExec[i]) {
			Execute(LocalExec[i], Modules[i].instname, modopts, expdaff);
			continue;
		}
...
INTERNAL int
checkmod(char *dir, char *modname, char *execfile) {
	char  dirp[4000];
	char *path;
	int  i;
	...
	/* Check for module in given directory - with and w/o .exe */
	sprintf(execfile, "%s%s", dirp, modname);

	if (access(execfile, X_OK) == SYS_OK) return(1);
	...
	while (path && *path) {
		for (i = 0; path[i] != ':'; ++i) dirp[i] = *path++;

		sprintf(execfile, "%s%s", dirp, modname);
		if (access(execfile, X_OK) == SYS_OK) return(1);

		sprintf(execfile, "%s%s.exe", dirp, modname);
		if (access(execfile, X_OK) == SYS_OK) return(1);
	}
	return(0);
 ...
 */
INTERNAL int
Execute(char *exec, char *instname, char *cmdopts, char *server) {
	char cmd[5000];

	sprintf(cmd, "%s -inst %s %s %s >/dev/null &",
		exec,
		instname,
		cmdopts,
		server);
...
/*
 * Fail - this should log a message back to the control layer
 */
INTERNAL void
Fail(char *pgm, char *msg) {
	fprintf(stderr, "%s: %s\n", pgm, msg);
...
usage(char *pgm, int msg) {
	...
	PRT("[-debug]",         "turning on loadexp debugging");
	PRT("[-moddebug]",      "increase module debugging level");
	PRT("[-modverbose]",    "increase module verbosity level");
	PRT("[-modtransport]",  "increase module transport debugging level");
	PRT("[-noconsole]",     "don't start a console");
	PRT("[-console]",       "start a console [default]");
	PRT("[-tunnel]",        "use tunnels for data transfer");
	PRT("[-notunnel]",      "don't tunnels for data transfer [default]");
	PRT("[-dump]",          "dump/capture stream data");
	PRT("[-nodump]",        "don't dump stream data [default]");
	PRT("[-output dir]",    "specify output directory");
	PRT("[-agentd]",        "start agentd on local host");
	PRT("[-noagentd]",      "do not start agentd on local host [default]");
	PRT("[-fileshare dir]", "start fileserver for specified directory");
	PRT("[-modules dir]",   "specify module executable directory");
	PRT("[-wd dir]",        "specify module working directory");
	PRT("[-logserver host]","specify (event) server host for logging");
	PRT("[-port n]",        "specify experiment server port");
	PRT("[-exec {local|ssh|agent]", "specify module execution mode");

	PRT("exp_server",       "experiment server name or \".\" to start locally]");
	PRT("expname",          "experiment name");
	PRT("master_server",    "earlab master server or -private or -search");
