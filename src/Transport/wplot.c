...
#define MAXCHANN   1024
#define MAXDATA    16000*MAXCHANN
...
INTERNAL int    readfile(char *file);
...
main(int argc, char *argv[]) {
	...
	int       backxs, backys;               /* backing pixmap size  */
	...
	int       plotx1, ploty1;               /* limits of plot area   */
	int       plotys;                       /* size of plot area     */
	...
	while (!Done) {
		...
		plotx1 = xs - 1;
		ploty1 = ys - 1;
		plotys = ploty1-ploty0+1;

		yscale = plotys * dscale;
		...
		if (PlotType & T_COLORMAP) {
			if (!img) for (i = 0; i < min(xs, nitem); ++i) {
				...
				yband = (float)(plotys) / (float)Nchan;
				...
				for (k = 0; k < Nchan; ++k) {
					...
					v = datum(Data, x0+i, k, DownSample);
			  ...
			  xw_showimage(img, imgxs, imgys, 0, ys-1, 0);
			}
		}
			...
			if (PlotType & T_STACKED) ploty0 = k*(float)plotys/(float)Nchan;

			for (i = 0; i < min(plotx1+1, nitem); i += DownSample) {
				v  = datum(Data, x0+i, k, DownSample);
				ix = plotx0 + i;
		...
		sprintf(text, "%d", min(nitem, x0+xs*DownSample-1));
		xw_qtext(text, &tw, &th, &ta, &td);
...
readdatum(FILE *f, float *datum) {
	...
	if (BinDouble) {
		...
		*datum = d;
		...
		rc = fread(datum, sizeof(float), 1, f);
	}
	else {
		float d;
...
do_menu(int item) {
	...
	if (SAMP_MENU_1 <= item && item <= SAMP_MENU_N) {
		CheckMenuItem(MainMenu, SAMP_MENU_0+DownSample,
			      MF_BYCOMMAND|MF_UNCHECKED);

		CheckMenuItem(MainMenu, item,
			      MF_BYCOMMAND|MF_CHECKED);

		DownSample = item-SAMP_MENU_0;
		Redraw = 1;
		return(1);
	}
