...
INTERNAL char *getpids(void);
INTERNAL char *mkpids(MP_PID_T pid, MP_PID_T tid);
...
printf("Initializing agent control block\n");
	...
	if (Debug)
	   printf("agentman: starting as thread %sn", getpids());
	...
	if (Debug)
	   printf("agentman: thread (%s) done and exiting\n", getpids());
...
start_logger(AGENT_REQ *areq, char *logserver, char *cmd, char *args, int *retpid) {
	...
	if (Debug)
	   printf("starting logger as proc %s for server %s\n", getpids());
	 ...
	 *  - start agent task
	 */
	fprintf(stderr, "logger: running as child proc %s\n", getpids());
	event_exit_();
	fprintf(stderr, "logger: proc %s - event_exit OK\n", getpids());
	fflush(stderr);
	mp_fini_();
	fprintf(stderr, "logger: proc %s - mp_fini OK\n", getpids());
	fflush(stderr);

	mp_init();
   /*   mp_trace(1);  */
	fprintf(stderr, "logger: proc %s - mp_init OK\n", getpids());
	fflush(stderr);

	fprintf(stderr, "logger: running as proc %s\n", getpids());
	fflush(stderr);
	Exitlock = mp_alloclock();
	Exitwait = mp_allocsema();
	...
	fprintf(stderr, "logger: %s joined %s as client %d\n",
		getpids(), logserver, id);

	event_register(LOGGERCLASS, LOGGERSPEC, cmd);

	rc = start_agent(NULL, cmd, args, retpid);

	fprintf(stderr, "logger: %s done and exiting with %d\n",
		getpids(), rc);
...
start_logger(AGENT_REQ *areq, char *logserver, char *cmd, char *args, int *retpid) {
	...
	char opts[2*AGENTD_PATHLEN];
	char opt[AGENTD_PATHLEN];
	int  pid;
	int  rc;

	opts[0] = '\0';

	/* Construct working and execution directory path options */
	if (WorkDir)  {
		sprintf(opt, "-wd %s ", WorkDir);
		strcat(opts, opt);
	}
	if (ExecPath) {
		sprintf(opt, "-bin %s ", ExecPath);
		strcat(opts, opt);
	}
...
/**************************/
	...
	if (Verbose) printf("start_agent: %s waiting for 2 %d.readers to exit\n", getpids(), childpid);
	mp_decsema(Exitwait);
	if (Verbose) printf("start_agent: %s waiting for 1 %d.reader to exit\n", getpids(), childpid);
	mp_decsema(Exitwait);
	if (Verbose) printf("start_agent: %s all %d.readers exited\n", getpid(), getpids(), childpid);
 ...
 ****/
	...
	if (Verbose)
	  printf("reader: proc %s monitoring %d.%s on fd %d, sending to client %d\n",
		   getpids(),
		   elog->pid,
...
/*
 * Incredibly stupid routine for dealing with printing of 64-bit thread ids
 * NOTE particularly reentrant, but faked to act so in simple cases
 *
 * Use a wrap around buffer, so we can have multiple concurrent calls
 * w/o memory collision.
 * Allocating the lock within the code is incredibly stupid, but will
 * work most of the time.
 */
INTERNAL char *getpids(void) {
	return(mkpids(getpid(), mp_gettid()));
}

INTERNAL char *
mkpids(MP_PID_T pid, MP_PID_T tid) {
	static char buf[4000], *bp = buf;
	static int  lock = 0;
	char   *rstr;

	if (!lock) lock = mp_alloclock(); /* completely bogus */

	mp_lock(lock);  /* pretend like we are actually rentrant */
	if (bp > buf+3950) bp = buf; /* wrap buffer near end */
	if (sizeof(MP_PID_T) != sizeof(int))
	   sprintf(bp, "%u.%llu", pid, tid);
	else
	   sprintf(bp, "%u.%u", pid, tid);
	rstr = bp;
	bp += strlen(bp)+1;
	mp_unlock(lock);
