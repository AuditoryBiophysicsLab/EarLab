...
main(int argc, char *argv[]) {
	...
	while (argc && argv[0][0] == '-') {
		...
		if (substr(opt, "help") || streq(opt, "?")) help = 1;
...
usage(char *pgm, int msg) {
	...
	PRT("[-channel n]",     "specify number of channels");
	PRT("[-select first [last]", "select input samples");
	PRT("[-mode {point|impulse|average|mean|absolute|RMS|minimum|maximum}]", "sampling mode");
