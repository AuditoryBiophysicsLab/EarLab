...
INTERNAL int   SetWD   = 1;
INTERNAL char *Fileshare = NULL;
INTERNAL char *ParamFile = NULL;
...
INTERNAL int   Dump   = 0;
INTERNAL int   LocalEx= 0;
INTERNAL int   Done   = 0;
INTERNAL int   Redraw = 0;
INTERNAL int   NeedDraw = 0;
...
INTERNAL void  set_state(char type, int indx, int state, Uint64 now);
INTERNAL void  show_init(void);
INTERNAL void  show_update(void);
...
main(int argc, char *argv[]) {
	...
	EVENT             e;
	Uint64 now, lastdraw = 0, lastwatch = 0;
	...
	ProbeName[PROBE_TYPE_WAVE   - PROBE_TYPE_0] = "viewwave";
	ProbeName[PROBE_TYPE_XWAVE  - PROBE_TYPE_0] = "xviewwave";
	ProbeName[PROBE_TYPE_SPIKE  - PROBE_TYPE_0] = "viewspike";
	ProbeName[PROBE_TYPE_XSPIKE - PROBE_TYPE_0] = "xviewspike";
	ProbeName[PROBE_TYPE_STRUCT - PROBE_TYPE_0] = "probe3d";
	...
	while (argc && *argv[0] == '-') {
		...
		else if (substr(opt, "localexec"))LocalEx = 1;
...
******************/
	...
	CheckMenuItem(MainMenu, OPT_MENU_LOCALEX,LocalEx?MF_CHECKED:MF_UNCHECKED);
	...
	while (!Done) {
		...
		if (now - lastwatch > 1000) {
			if (watch_file(Config)) load_config(Config);
			lastwatch = now;
		}

		/* Check to see if we need to redraw everything */
		if (Redraw) {
			show_drawall();
			Redraw   = 0;
			NeedDraw = 0;
		...
		/* Check for redraw every 50 milliseconds */
		if (now - lastdraw > 50 && NeedDraw) {
			show_update();
			lastdraw = now;
		}
...
Connect(char *server) {
	...
	/* Select only state and system messages */
	event_select_type(0, ET_USER_MIN,   ET_MAX);
...
load_config(char *config) {
	...
	if (s = strrchr(Config, '/')) {
		if (SetWD) {
			...
			WorkDir = (char *)malloc(n+1);
			for (i = 0; i < n; ++i) WorkDir[i] = Config[i];
			WorkDir[i] = '\0';
		     printf("Set working directory to \"%s\"\n", WorkDir);
		}
		strcpy(ConfigName, s+1);
	}
...
usage(char *pgmname, int msg) {
	...
	PRT("[-verbose]",       "enable verbose messages");
	PRT("[-quiet]",         "disable verbose messages");
	...
	PRT("[-capture]",       "capture all transmitted module data");
	PRT("[-dump]",          "synonymous with \"-capture\"");
	PRT("[-nodump]",        "disable data capture (dump)");
	PRT("[-dir dir]",       "use dir as file working directory");
	PRT("[-cd dir]",        "synonomous with -dir");
#ifdef WINAPP
...
show_update(void) {
	...
	for (i = 0; i < Nmodules; ++i)
	 if (ModState[i].lastchange > ModState[i].lastdraw)
	  show_draw(i);
...
show_draw(int imod) {
	...
	ModState[imod].lastdraw = event_clock();
...
INTERNAL void
set_state(char type, int indx, int state, Uint64 now) {
	int x0, y0, x1, y1;
	int i;

	if (type == 'M') {
		ModState[indx].state      = state;
		ModState[indx].lastchange = now;
	    /****
	    ****/ NeedDraw = 1;
		if (ModState[indx].lastchange - ModState[indx].lastdraw > 100)
		    show_draw(indx);
		else NeedDraw = 1;
		if (Debug)
...
do_run_run(void) {
	...
	if (WorkDir)   sprintf(dir,   "-dir \"%s\"", WorkDir);
	if (ParamFile) sprintf(param, "\"%s\"", ParamFile);
	sprintf(buf,
		"runmodel -steps %d %s %s %s %s \"%s\" &",
		Nstep, dir,
		Unload?"-unload":"-nounload",
...
do_menu(int item) {
	...
	if (PROBE_MENU_PROBE_0 <= item && item <= PROBE_MENU_PROBE_N) {
		...
		sprintf(buf, "%s -autopan -dim %s daffie://%s/%s &",
			ProbeExec, dimstr, Server, probename);
	...
	switch (item) {
	  ...
	  case DLG_MASTER_ENTER:
		...
		if (Master) {
			...
			sprintf(buf, "Could not find master \"%s\" on host \"%s\"\n",
				world, host);
		}
		StatusSelect(3);
		StatusMsg(buf);
	  ...
	  case RUN_MENU_LOAD:
		...
		if (LocalEx)                   strcat(opts, " -exec local");
		if (WorkDir) {
			strcat(opts, " -wd ");
			strcat(opts, WorkDir);
		}
	  ...
	  case OPT_MENU_LOCALEX:
		LocalEx = !LocalEx;
		CheckMenuItem(MainMenu, OPT_MENU_LOCALEX,
			      LocalEx?MF_CHECKED:MF_UNCHECKED);
		return(1);
...
SearchWorld(char *meta, char *name, char *host) {
	...
	printf("SearchWorld: connected to Metaworld at %s with %d clients\n",
		meta, nc);
