...
main(int argc, char *argv[]) {
	...
	/* See if parameter file name was given */
	if (argc) {
		ParamFile = argv[0];
		++argv; --argc;
	}

	/* Change to specified working directory if specified */
	if (WorkDir) {
		rc = chdir(WorkDir);
		if (rc < 0) {
			fprintf(stderr,
				"%s: couldn't change to directory \"%s\"\n",
				Pgm, WorkDir);
			exit(1);
		}
	}

	if (!DefineModel(DiagFile)) {
		fprintf(stderr,
	...
	TransportInit(server, ExpName?ExpName:DiagFile);
...
SetAllParameters(void) {
	...
	****************************************************/
	...
	/* Get the parameter file name - right now this is a hack */
	if (ParamFile) paramfile = ParamFile;
	else           paramfile = ModuleConf[0].paramf;

	/* Initialize parameter file */
	InitParameterFile();

	/* Read the parameter file */
	rc = ReadParameterFile(paramfile);
	if (!rc) {
		Error = 1;
 ...
 /****
  **** N.B. not sure if this should go before the ParameterFileRead
  **** which would give precedence to values in parameter file.
  **** The way it is, command line args override parameter file
  ****/

	/* Set model level parameters */
	SetParameterString("Model.ParameterFile", paramfile);
	SetParameterString("Model.DiagramFile",   DiagFile);
	SetParameterInt("Model.Steps",            Nstep);
	if (WorkDir)
	   SetParameterString("Model.WorkingDirectory", WorkDir);

	if (ExpName)
	   SetParameterString("Model.ExperimentName", ExpName);


	/* Broadcast all the parameter values */
	SendParameters(EVENT_BCAST_NOTME);

	/* Send a EFI_PARAMETER_FILE message */
	strcpy(epf.filename, "");
	strcpy(epf.section,  "");

	event_send(EVENT_BCAST_NOTME, ET_EFI_PARAMETER_FILE,
		   (EVENT *)&epf, sizeof(EVENT_EFI_PARAMETER_FILE));
