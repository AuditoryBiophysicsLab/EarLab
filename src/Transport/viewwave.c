#if (defined(WINAPP) || defined(win32))
...
INTERNAL int    Done = 0;
INTERNAL int    Loop = 1;               /* Loop on end-of-file          */
...
main(int argc, char *argv[]) {
	...
	int       plotx1, ploty1;               /* limit of plot area    */
	UTime     now;

	Pgm = argv[0];  if (streq(Pgm, "app_main")) Pgm = "viewwave";
	++argv; --argc;
	...
	while (argc && *argv[0] == '-') {
		...
		else if (substr(opt, "loop"))        Loop    = 1;
		else if (substr(opt, "noloop"))      Loop    = 0;
...
#if (defined(XWINDOWS) && !defined(OGL))
	/* Create back buffer */
	if (Double) {
		int xs, ys;
	...
	while (!Done) {
		...
		if (PlotType & T_COLORMAP) {
			...
			draw = (img == NULL);
#endif
			if (draw) for (i = 0; i < min(xs, nitem); ++i) {
				float yband;
				int    iy;

				 yband = (float)(ploty1-ploty0+1) / (float)Nchan;
