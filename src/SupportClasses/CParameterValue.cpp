...
CParameterValue::CParameterValue(const char *StringVal)
{
	...
	strncpy(mValue.StringVal, StringVal, strlen(StringVal) + 1);
}
...
ParameterStatus CParameterValue::GetValue(double *RetVal, int &ArrayLen)
{
	...
	else
	{
		...
		for (i = 0; (i < ArrayLen) && (i < mArrayLength); i++)
			RetVal[i] = mValue.DoubleVectorVal[i];
		ArrayLen = i;
		if (mArrayLength > ArrayLen)
			return BufferOverflowSuccess;
		else
...
const char *CParameterValue::ValueToString(void)
{
	...
	case IntVector:
		...
		for (i = 0; i < mArrayLength - 1; i++)
		{
			sprintf(TempValueString, "%d ", mValue.IntVectorVal[i]);
			strcat(mValueString, TempValueString);
		}
		sprintf(TempValueString, "%d", mValue.IntVectorVal[mArrayLength - 1]);
		strcat(mValueString, TempValueString);
		return mValueString;
	case DoubleVector:
		...
		sprintf(TempValueString, "%f", mValue.DoubleVectorVal[mArrayLength - 1]);
		strcat(mValueString, TempValueString);
		return mValueString;
	case Uninitialized:
		return "Type not set";
