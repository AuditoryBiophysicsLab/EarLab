#include "Earlab.h"
#include "ZLSynapseSecondOrder.h"

ZLSynapseSecondOrder::ZLSynapseSecondOrder(double SampleRate_Hz, double Tau, double EPSP_strength)
{
	this->first = 0.0;
	this->second = 0.0;
	this->in = 0.0;
...
//	this->EPSP_ScaleFactor_Siemens = (EPSP_strength * 1.0e-9) * (E / Tau);
	...
	// In ZLSynapses there is no 1.0e-9 conversion from nS to S (as in the HHSynapse).
	// The factor (E / Tau) is maintained with a 2nd order (alpha function) conductance profile
}

void ZLSynapseSecondOrder::Step(void)
{
