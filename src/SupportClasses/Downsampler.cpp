						 ...
						 const char *OutputFilename, const char *CFFileName, const char *ModuleInstanceName)
{
	...
	if (strlen(OutputFilename) > 245)
		throw EarlabException("Downsampler: File name is too int (%d bytes)", strlen(OutputFilename));
...
void Downsampler::SetMinimumSampleCountToKeep(unsigned int MinimumSampleCountToKeep)
{
...
void Downsampler::Flush(float *Buffer)
{
	...
	if ((mMetadataFile->GetSampleCount() >= mMinimumSampleCountToKeep) || (mDownsampleLevel == 0))
	{
