...
void RungeKuttaSolver::ode_rk(int nvar, double x1, double x2, double eps, double h1, double hmin, int *nok, int *nbad, RungeKuttaEquation *Equations[])
{
	...
	*nok = (*nbad) = kount = 0;
	...
	for (nstp=1;nstp<=MAXSTP;nstp++)
	{
		...
		for (i = 1; i <= nvar ; i++)
			dydx[i] = Equations[i]->GetDerivative(y[i], y[nvar]);
...
double *RungeKuttaSolver::new_vector(int nl, int nh)
{
	double *v;
...
double **RungeKuttaSolver::new_matrix(int nrl, int nrh, int ncl, int nch)
{
...
void RungeKuttaSolver::delete_vector(double *v, int nl, int nh)
{
	free((void *)(v+nl-NR_END));
}

void RungeKuttaSolver::delete_matrix(double **m, int nrl, int nrh, int ncl, int nch)
{
