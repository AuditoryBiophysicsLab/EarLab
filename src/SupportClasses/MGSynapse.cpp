...
MGSynapse::MGSynapse(double SampleRate_Hz, double Tau, double EPSP_strength)
{
	this->first = 0.0;
	this->second = 0.0;
	this->in = 0.0;
	...
	// The factor (E / Tau) is maintained when assuming a 2nd order (alpha function) conductance profile
	this->EPSP_ScaleFactor_Siemens = EPSP_strength * (E / Tau);
}

void MGSynapse::Step(void)
{
